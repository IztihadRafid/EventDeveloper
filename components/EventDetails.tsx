...
import { Building2, Calendar, ClipboardList, Clock3, MapPin, Monitor, Tag, User, Users } from 'lucide-react';
...
const EventDetails = async({params}:{params:Promise<string>}) => {
  ...
  return (
    <section>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 lg:grid-cols-3">
          ...
          <div className="space-y-8 lg:col-span-2">
            ...
            <section className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
              ...
              <p className="leading-8 text-gray-300">{event?.description}</p>
            </section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20">
        <div className="overflow-hidden rounded-3xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 via-white/5 to-white/5 p-8 backdrop-blur">
          <div className="mb-8 text-center">
            <h2 className="text-3xl font-bold text-white">Book Your Spot</h2>

            {bookings > 0 ? (
              <p className="mt-3 text-gray-300">
                Join{" "}
                <span className="font-semibold text-cyan-400">{bookings}</span>{" "}
      ...
      <section>
        <h2 className="text-4xl text-center p-10 mt-14">Similar Events</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8 mb-6 w-[80%] mx-auto">
          {similarEvents.length > 0 &&
            similarEvents.map((similarEvent) => (
              <div className="w-96" key={similarEvent.title}>
                <EventCard {...similarEvent}></EventCard>
              </div>
            ))}
        </div>
      </section>
